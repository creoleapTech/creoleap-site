  ...
  const handleSubmit = async (event: { preventDefault: () => void; target: HTMLFormElement | undefined; }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(responseData.error || 'Failed to send email. Please try again.');
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-6xl mx-auto relative z-10">
        ...
        <div className="space-y-6">
          <form className="space-y-4 sm:space-y-6 pr-0 md:pr-8" onSubmit={handleSubmit}>
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold">Book a Quick Consultation with Us</h2>
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm bg-[#1A023E] text-white focus:outline-none focus:ring-2 focus:ring-white disabled:opacity-50"
            ...
            <div ref={dropdownRef}>
              <label htmlFor="interestedServices" className="block text-sm font-medium text-gray-300">
                Interested Services*
              </label>
              <div className="relative">
                <div
                  className="w-full mt-1 px-3 py-2 border border-gray-600 rounded-md shadow-sm bg-[#1A023E] text-white focus:outline-none focus:ring-2 focus:ring-white disabled:opacity-50 cursor-pointer flex items-center justify-between"
                ...
                {isDropdownOpen && (
                  <div className="absolute z-20 w-full mt-1 bg-[#1A023E] border border-gray-600 rounded-md shadow-lg text-white max-h-60 overflow-y-auto">
            ...
            <div>
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-3 rounded-md hover:from-blue-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-white disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
              >
                {isSubmitting ? (
